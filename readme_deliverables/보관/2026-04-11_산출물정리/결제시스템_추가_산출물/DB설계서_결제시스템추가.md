...
| v1.2 | 2026.03.28 | - | 결제 시스템 추가 — payment 테이블 컬럼 4개 신규 추가 (provider, payment_key, installment_months, failure_reason) |
...
| category_top | category_status | ENUM | | | ❌ | 'ACTIVATE' | ACTIVATE, DEACTIVATE, DELETE |
...
| category_sub | category_status | ENUM | | | ❌ | 'ACTIVATE' | ACTIVATE, DEACTIVATE, DELETE |
...
| payment | **payment_key** | **VARCHAR(200)** |  |  | **✅** | **null** | **토스 결제 고유 키 (payment_key) — v1.2 신규** |
...
| KAKAO | 카카오페이 |
| NAVER | 네이버페이 |
| null (기존) | Mock PG 또는 provider 미설정 건 |
...
erDiagram
    ...
    PAYMENT {
        bigint id PK
        bigint order_id FK
        string provider "TOSS | KAKAO | NAVER"
        string method
        enum payment_status "READY | PAID | CANCELLED | FAILED"
        int amount
        string pg_tid
        string payment_key
        smallint installment_months
        datetime paid_at
...
#### 📌 결제 도메인 (v1.2 — 결제 시스템 추가 반영)

```sql
-- v1.2 변경: provider, payment_key, installment_months, failure_reason 컬럼 추가
CREATE TABLE IF NOT EXISTS payment (
    id                 BIGSERIAL        PRIMARY KEY,
    order_id           BIGINT           NOT NULL REFERENCES "order" (id),
    provider           VARCHAR(20),                          -- TOSS | KAKAO | NAVER (신규)
    method             VARCHAR(30)      NOT NULL,
    payment_status     payment_status   NOT NULL DEFAULT 'READY',
    amount             INT              NOT NULL DEFAULT 0,
    pg_tid             VARCHAR(100),
    payment_key        VARCHAR(200),                         -- 토스 payment_key (신규)
    installment_months SMALLINT,                             -- 할부 개월 수 (신규)
    paid_at            TIMESTAMP,
    cancel_reason      VARCHAR(300),
    failure_reason     VARCHAR(300),                         -- 결제 실패 사유 (신규)
    created_at         TIMESTAMP        NOT NULL DEFAULT now(),
    updated_at         TIMESTAMP,
    cancelled_at       TIMESTAMP
);
...
> **기존 테이블 ALTER (운영 환경 마이그레이션용)**
>
> ```sql
> ALTER TABLE payment ADD COLUMN IF NOT EXISTS provider           VARCHAR(20);
> ALTER TABLE payment ADD COLUMN IF NOT EXISTS payment_key        VARCHAR(200);
> ALTER TABLE payment ADD COLUMN IF NOT EXISTS installment_months SMALLINT;
> ALTER TABLE payment ADD COLUMN IF NOT EXISTS failure_reason     VARCHAR(300);
> ```
...
CREATE INDEX IF NOT EXISTS idx_payment_payment_key ON payment (payment_key) WHERE payment_key IS NOT NULL;
```
