...
| --- | --- | --- | --- | --- | --- |
| FR-M-001 | 로그인 | 이메일과 비밀번호로 로그인. JWT 토큰 발급 후 로컬 상태 관리 | 높음 🔴 | /signin | 로그인 실패 시 에러 메시지 노출. 이메일 미존재 / 비밀번호 불일치 구분 메시지 |
...
| FR-M-005 | 회원 탈퇴 | 계정 휴면 처리. memberStatus → DEACTIVATE 로 변경 | 중간 🟠 | /mypage/profile | 탈퇴 전 비밀번호 재확인. 탈퇴 시 deleted_at 기록 |
| FR-M-006 | 주문 내역 조회 | 마이페이지에서 최근 주문 내역 5건을 내림차순으로 노출 | 중간 🟠 | /mypage/order | 주문 상태 표시: 결제완료 / 주문승인 / 배송중 / 배송완료 |
| FR-M-007 | 장바구니 목록 조회 | 마이페이지에서 현재 장바구니에 담긴 상품 목록 조회 | 중간 🟠 | /mypage/cart | 수량 및 총 금액 표시. 장바구니 페이지로 이동 버튼 포함 |
| FR-M-008 | 리뷰 목록 / 수정 / 삭제 | 구매 완료된 상품에 작성한 리뷰 목록 조회, 수정, 삭제 | 낮음 🟢 | /mypage/review | 배송완료 상태의 주문 상품에 한해 리뷰 작성 가능. 본인 리뷰만 수정·삭제 가능 |
...
| FR-A-002 | 회원 목록 조회 | 전체 회원 목록을 최근 가입 순(created_at 내림차순)으로 조회. 페이징 처리 | 높음 🔴 | /admin/member | 이메일, 이름, 가입일, 권한, 상태 컬럼 표시 |
| FR-A-003 | 회원 검색 | 전체 / 이메일 / 이름 기준으로 회원 검색 후 목록 조회 | 높음 🔴 | /admin/member | 검색 조건 선택(셀렉트박스) + 키워드 입력. 검색 결과도 페이징 처리 |
| FR-A-004 | 회원 상세 정보 조회 | 특정 회원의 주문 내역, 리뷰 목록, QnA 목록 조회 | 중간 🟠 | /admin/member/:id | 각 항목별 탭(Tab) 구성 권장 |
| FR-A-005 | 회원 강퇴 | 특정 회원의 memberStatus → DEACTIVATED 처리 | 높음 🔴 | /admin/member/:id | 강퇴 전 확인 모달 필수. 강퇴된 회원은 로그인 불가 처리 |
| FR-A-006 | 신규 주문 승인 | 결제 완료 후 미승인 상태인 주문 목록 조회 및 승인 여부 결정 | 높음 🔴 | /admin/order/pending | 승인 시 주문 상태 → 주문승인 변경. 거절 시 사유 입력 후 환불 처리 연동 |
| FR-A-007 | 회원 등급 변경 | 특정 회원의 memberRole 변경 (USER → MANAGER → ADMIN) | 중간 🟠 | /admin/member/:id | ADMIN 권한만 등급 변경 가능. 본인 등급 변경 불가 |
