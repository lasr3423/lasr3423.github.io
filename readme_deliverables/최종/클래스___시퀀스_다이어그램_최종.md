...
classDiagram
    ...
    class AdminMemberApiController {
        ...
        +getMembers(Pageable, keyword, role, status) ResponseEntity
    ...
    class AdminOrderApiController {
        +getOrderList(Pageable, keyword, status) ResponseEntity
    ...
    class ReviewService {
        +getTopReviewsByProduct(Long) List
    ...
    class MemberRepository {
        ...
        +findAllByKeywordAndRoleAndStatus(Pageable, String, MemberRole, MemberStatus) Page
    }
    ...
    class OrderRepository {
        ...
        +countByMemberIdAndOrderStatus(Long, OrderStatus) long
        +findAllByKeywordAndStatus(Pageable, String, OrderStatus) Page
    }
    ...
    class ReviewRepository {
        ...
        +findTop5ByProductIdAndDeletedAtIsNull(Long) List
    }
    ...
    MyPageApiController --> MemberService
    ProductApiController --> ProductService
    ProductApiController --> ReviewService
...
sequenceDiagram
    ...
    Client->>AC: POST /signup { email, password, name, phone, address, marketingAgreed }
    ...
    alt 이메일 중복
        ...
        MS->>MR: save(Member)
        MR->>DB: INSERT INTO member (email, password, name, phone, address, member_role, member_status, marketing_agreed)
        DB-->>MR: saved Member
        MR-->>MS: Member
        MS-->>AC: void
...
sequenceDiagram
    ...
    Note over Client, DB: [비로그인] 비밀번호 찾기 → 재설정
    Client->>AC: PATCH /auth/reset-password { email, newPassword, confirmPassword }
    AC->>MS: resetPassword(ResetPasswordRequest)
    ...
    alt 회원 미존재
        MR-->>MS: Optional.empty()
        MS-->>AC: throw MemberNotFoundException
        AC-->>Client: 404 Not Found "존재하지 않는 이메일입니다."
    else 회원 존재
        MR-->>MS: Optional[Member]
        MS->>MS: newPassword.equals(confirmPassword) 검증
        alt 비밀번호 불일치
            MS-->>AC: throw PasswordMismatchException
            AC-->>Client: 400 Bad Request "새 비밀번호가 일치하지 않습니다."
        else 일치
            MS->>MS: BCrypt.encode(newPassword)
            MS->>MR: save(member.updatePassword(encodedPw))
            MR->>DB: UPDATE member SET password = ? WHERE id = ?
            DB-->>MR: updated
            MS-->>AC: void
            AC-->>Client: 200 OK "비밀번호가 변경되었습니다."
        end
    end

    Note over Client, DB: [로그인] 마이페이지 비밀번호 변경
    Client->>AC: PATCH /mypage/password { newPassword, confirmPassword }
    Note over AC: JWT 인증 → memberId 추출
    AC->>MS: updatePassword(memberId, PasswordRequest)
    MS->>MS: newPassword.equals(confirmPassword) 검증
    alt 비밀번호 불일치
        MS-->>AC: throw PasswordMismatchException
        AC-->>Client: 400 Bad Request "새 비밀번호가 일치하지 않습니다."
...
sequenceDiagram
    ...
    and 신규 회원 집계
        ...
        DB-->>MR: Long count
    and 카테고리별 매출 집계
        AS->>OR: findCategorySales()
        OR->>DB: SELECT ct.name, SUM(oi.item_total), COUNT(oi.id) FROM order_item oi JOIN product p ON p.id = oi.product_id JOIN category_top ct ON ct.id = p.category_top_id WHERE oi.order.order_status != 'CANCELED' GROUP BY ct.name
        DB-->>OR: List~CategorySalesDto~
    and 월별 매출 집계 (최근 6개월)
        AS->>OR: findMonthlySales(startDate)
        OR->>DB: SELECT TO_CHAR(ordered_at, 'YYYY-MM'), SUM(final_price) FROM order WHERE order_status != 'CANCELED' AND ordered_at >= ? GROUP BY 1 ORDER BY 1
        DB-->>OR: List~MonthlySalesDto~
    end

    AS-->>ADC: DashboardResponse { pendingOrders, waitingQnas, lowStockProducts, todaySales, monthSales, newMembersToday, newMembersMonth, categorySales, monthlySales }
