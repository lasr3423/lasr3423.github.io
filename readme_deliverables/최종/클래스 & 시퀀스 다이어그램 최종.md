# 클래스 & 시퀀스 다이어그램 최종

| 항목 | 내용 |
| --- | --- |
| 프로젝트명 | 교보문고 쇼핑몰 시스템 (README) |
| 개발 환경 | Java 21 / Spring Boot 3.5 / PostgreSQL / Vue.js 3 |
| 작성일 | 2026-03-24 |
| 최종 수정 | 2026-03-29 (v1.3) |
| 연관 문서 | DB설계서 최종.md / 유스케이스 명세서 최종.md / 기능명세서 최종.md |

## 📋 변경 이력

| 버전 | 변경일 | 변경 내용 |
| --- | --- | --- |
| v1.0 | 2026-03-24 | 최초 작성 |
| v1.2 | 2026-03-28 | Payment 엔티티 필드 4개 추가 (provider, paymentKey, installmentMonths, failureReason) / PaymentProvider enum 추가 / PaymentGateway 인터페이스 및 구현체 3개 추가 / AdminService 클래스 추가 / CategoryService·CategoryRepository 추가 / 로그아웃 시퀀스 추가 / PG사별 결제 시퀀스 추가 |
| v1.3 | 2026-03-29 | Member 엔티티 OAuth 필드 추가 (provider, providerId) / AuthProvider enum 추가 / RefreshToken 엔티티 신규 추가 / Security 계층 클래스 추가 (JwtAuthenticationFilter, CustomUserDetails, CustomUserDetailsService, SecurityConfig) / OAuth2 계층 클래스 추가 (CustomOAuth2UserService, OAuth2SuccessHandler, OAuth2FailureHandler, KakaoOAuthService, OAuthMemberService) / KakaoOAuthController 추가 / RefreshTokenRepository 추가 / MemberService refresh·issueTokens 메서드 추가 / AuthController refresh 메서드 추가 / 토큰갱신·구글OAuth·카카오OAuth 시퀀스 다이어그램 추가 |

---

## 📐 1. 클래스 다이어그램

### 1-1. Entity 전체 구조 (핵심 관계)

```mermaid
classDiagram
    direction TB

    class BaseEntity {
        <<abstract>>
        +Long id
        +LocalDateTime createdAt
        +LocalDateTime updatedAt
    }

    class Member {
        +String email
        +String password
        +String name
        +String phone
        +String address
        +MemberRole memberRole
        +MemberStatus memberStatus
        +AuthProvider provider
        +String providerId
        +LocalDateTime deletedAt
    }

    class RefreshToken {
        +Long memberId
        +String token
        +LocalDateTime expiresAt
    }

    class CategoryTop {
        +String name
        +Integer sortOrder
        +CategoryStatus categoryStatus
    }

    class CategorySub {
        +String name
        +Integer sortOrder
        +CategoryStatus categoryStatus
    }

    class Product {
        +String title
        +String author
        +String description
        +Integer price
        +BigDecimal discountRate
        +Integer salePrice
        +Integer stock
        +String thumbnail
        +Integer viewCount
        +Integer salesCount
        +ProductStatus productStatus
        +LocalDateTime deletedAt
    }

    class ProductImage {
        +String imageUrl
        +String type
        +Integer sortOrder
    }

    class Cart {
    }

    class CartItem {
        +Integer quantity
        +Boolean isChecked
    }

    class Order {
        +String number
        +OrderStatus orderStatus
        +Integer totalPrice
        +Integer discountAmount
        +Integer finalPrice
        +String receiverName
        +String receiverPhone
        +String deliveryAddress
        +String deliveryMemo
        +LocalDateTime orderedAt
        +LocalDateTime cancelledAt
    }

    class OrderItem {
        +String productTitle
        +String productAuthor
        +Integer salePrice
        +Integer quantity
        +Integer itemTotal
        +Boolean isReviewed
    }

    class Payment {
        +String method
        +String provider
        +PaymentStatus paymentStatus
        +Integer amount
        +String pgTid
        +String paymentKey
        +Integer installmentMonths
        +LocalDateTime paidAt
        +String cancelReason
        +String failureReason
        +LocalDateTime cancelledAt
    }

    class Delivery {
        +String courier
        +String trackingNumber
        +DeliveryStatus deliveryStatus
        +LocalDateTime shippedAt
        +LocalDateTime deliveredAt
    }

    class Notice {
        +String title
        +String content
        +Boolean isFixed
        +Integer viewCount
        +LocalDateTime deletedAt
    }

    class Qna {
        +Integer depth
        +String category
        +String title
        +String content
        +Integer viewCount
        +QnaStatus qnaStatus
        +Boolean isSecret
        +LocalDateTime answeredAt
        +LocalDateTime deletedAt
    }

    class Review {
        +Integer rating
        +String content
        +Integer hits
        +LocalDateTime deletedAt
    }

    class ReviewImage {
        +String imageUrl
    }

    class ReviewReaction {
        +String reactionType
    }

    %% ENUM 클래스
    class MemberRole {
        <<enumeration>>
        USER
        MANAGER
        ADMIN
    }

    class MemberStatus {
        <<enumeration>>
        ACTIVATE
        DEACTIVATE
        DELETE
    }

    class CategoryTopStatus {
        <<enumeration>>
        ACTIVATE
        DEACTIVATE
        DELETE
    }

    class CategorySubStatus {
        <<enumeration>>
        ACTIVATE
        DEACTIVATE
        DELETE
    }

    class ProductStatus {
        <<enumeration>>
        ACTIVATE
        DEACTIVATE
        DELETE
    }

    class OrderStatus {
        <<enumeration>>
        PENDING
        PAYED
        APPROVAL
        CANCELED
    }

    class PaymentStatus {
        <<enumeration>>
        READY
        PAID
        CANCELLED
        FAILED
    }

    class PaymentProvider {
        <<enumeration>>
        TOSS
        KAKAO
        NAVER
    }

    class AuthProvider {
        <<enumeration>>
        LOCAL
        GOOGLE
        KAKAO
    }

    class DeliveryStatus {
        <<enumeration>>
        READY
        SHIPPED
        IN_TRANSIT
        DELIVERED
        FAILED
    }

    class QnaStatus {
        <<enumeration>>
        WAITING
        PROCESSING
        COMPLETE
    }

    %% BaseEntity 상속
    BaseEntity <|-- RefreshToken
    BaseEntity <|-- Member
    BaseEntity <|-- CategoryTop
    BaseEntity <|-- CategorySub
    BaseEntity <|-- Product
    BaseEntity <|-- Cart
    BaseEntity <|-- CartItem
    BaseEntity <|-- Order
    BaseEntity <|-- OrderItem
    BaseEntity <|-- Payment
    BaseEntity <|-- Delivery
    BaseEntity <|-- Notice
    BaseEntity <|-- Qna
    BaseEntity <|-- Review
    BaseEntity <|-- ReviewImage
    BaseEntity <|-- ReviewReaction

    %% 연관 관계
    Member "1" --> "1" Cart : has
    Member "1" --> "0..*" Order : places
    Member "1" --> "0..*" Notice : writes
    Member "1" --> "0..*" Qna : asks
    Member "1" --> "0..*" Review : writes
    Member "1" --> "0..*" ReviewReaction : reacts

    CategoryTop "1" --> "0..*" CategorySub : contains
    CategoryTop "1" --> "0..*" Product : classifies
    CategorySub "1" --> "0..*" Product : classifies

    Product "1" --> "0..*" ProductImage : has
    Product "1" --> "0..*" CartItem : in cart
    Product "1" --> "0..*" OrderItem : ordered
    Product "1" --> "0..*" Review : reviewed

    Cart "1" --> "0..*" CartItem : contains
    Order "1" --> "0..*" OrderItem : contains
    Order "1" --> "1" Payment : paid by
    Order "1" --> "1" Delivery : shipped by

    Qna "0..1" --> "0..*" Qna : replies
    Review "1" --> "0..*" ReviewImage : has
    Review "1" --> "0..*" ReviewReaction : receives

    Member "1" --> "0..*" RefreshToken : issues

    Member ..> MemberRole : uses
    Member ..> MemberStatus : uses
    Member ..> AuthProvider : uses
    CategoryTop ..> CategoryTopStatus : uses
    CategorySub ..> CategorySubStatus : uses
    Product ..> ProductStatus : uses
    Order ..> OrderStatus : uses
    Payment ..> PaymentStatus : uses
    Payment ..> PaymentProvider : uses
    Delivery ..> DeliveryStatus : uses
    Qna ..> QnaStatus : uses
```

---

### 1-2. 레이어 아키텍처 클래스 구조

```mermaid
classDiagram
    direction LR

    class AuthController {
        +signup(SignupRequest) ResponseEntity
        +signin(SigninRequest) ResponseEntity
        +signout(HttpServletRequest) ResponseEntity
        +refresh(String refreshToken) ResponseEntity
    }

    class KakaoOAuthController {
        +authorize() ResponseEntity
        +callback(String code) ResponseEntity
    }

    class MemberController {
        +getMyPage(Member) ResponseEntity
        +updateInfo(UpdateRequest, Member) ResponseEntity
        +updatePassword(PasswordRequest, Member) ResponseEntity
        +withdraw(Member) ResponseEntity
        
        +getOrderList(Member, Pageable) ResponseEntity
        +getOrderDetail(Long, Member) ResponseEntity
        +cancelOrder(Long, Member) ResponseEntity
        
        +getPaymentList(Payment, Pageable) ResponeEntity
        +cancelPayment(Long, Member) ResponseEntity
        
        +getDeliveryStatus(Long, Member) ResponseEntity
        
        +getQnaList(Pageable) ResponseEntity
        +getQnaDetail(Long, Member) ResponseEntity
        +writeQuestion(QnaRequest, Member) ResponseEntity
    }

    class ProductController {
        +getProductList(Pageable, keyword) ResponseEntity
        +getProductDetail(Long) ResponseEntity
    }

    class CartController {
        +getCart(Member) ResponseEntity
        +addItem(CartItemRequest, Member) ResponseEntity
        +updateQuantity(CartUpdateRequest, Member) ResponseEntity
        +removeItem(Long, Member) ResponseEntity
    }

    class OrderController {
        +createOrder(OrderRequest, Member) ResponseEntity      
    }

    class PaymentController {
        +requestPayment(PaymentRequest, Member) ResponseEntity
        +completePayment(PaymentCompleteRequest) ResponseEntity
    }

    class DeliveryController {
        +getDeliveryStatus(Long, Member) ResponseEntity
    }

    class ReviewController {
		    +getReviewList(keyword, pageable) ResposeEntity
		    +getReviewDetail(keyword, pageable, Member) ResposeEntity
        +writeReview(ReviewRequest, Member) ResponseEntity
        +updateReview(Long, ReviewRequest, Member) ResponseEntity
        +deleteReview(Long, Member) ResponseEntity
       
        }

    class QnaController {
        +getQnaList(keyword,Pageable) ResponseEntity
        +getQnaDetail(Long, Member) ResponseEntity
        +writeQuestion(QnaRequest, Member) ResponseEntity
        +updateQuestion(Long, QnaRequest, Member) ResponseEntity
        +deleteQuestion(Long, Member) ResponseEntity
    }

    class NoticeController {
        +getNoticeList(Pageable) ResponseEntity
        +getNoticeDetail(Long) ResponseEntity
    }

    class AdminController {
        +getDashboard() ResponseEntity
        +getMembers(Pageable, keyword) ResponseEntity
    }

    class AdminProductController {
        +getProductList(Pageable, keyword) ResponseEntity
        +getProductDetail(Long) ResponseEntity
        +createProduct(ProductRequest) ResponseEntity
        +updateProduct(Long, ProductRequest) ResponseEntity
        +deleteProduct(Long) ResponseEntity
    }
    
    class AdminCategoryController {
        +getCategoryList(Pageable, keyword) ResponseEntity
    }

    class AdminOrderController {
        +getOrderList(Pageable, keyword) ResponseEntity
        +getOrderDetail(Long) ResponseEntity
    }

    class AdminPaymentController {
        +getPaymentList(Pageable) ResponseEntity
        +getPaymentDetail(Long) ResponseEntity
    }

    class AdminDeliveryController {
        +getDeliveryList(Pageable, status) ResponseEntity
        +getDeliveryDetail(Long) ResponseEntity
    }

    class AdminReviewController {
        +getReviewList(Pageable) ResponseEntity
        +getReviewDetail(Long) ResponseEntity
        +updateReview(Long, ReviewRequest) ResponseEntity
        +deleteReview(Long) ResponseEntity
    }

    class AdminQnaController {
        +getQnaList(Pageable, status) ResponseEntity
        +getQnaDetail(Long) ResponseEntity
        +answerQna(Long, AnswerRequest) ResponseEntity
        +updateAnswer(Long, AnswerRequest) ResponseEntity
        +deleteQna(Long) ResponseEntity
    }

    class AdminNoticeController {
        +getNoticeList(Pageable) ResponseEntity
        +getNoticeDetail(Long) ResponseEntity
        +createNotice(NoticeRequest) ResponseEntity
        +updateNotice(Long, NoticeRequest) ResponseEntity
        +deleteNotice(Long) ResponseEntity
    }

    class MemberService {
        +signup(SignupRequest) void
        +signin(SigninRequest) TokenResponse
        +signout(String) void
        +refresh(String refreshToken) TokenResponse
        +issueTokens(Member) TokenResponse
        +updateInfo(Long, UpdateRequest) void
        +updatePassword(Long, PasswordRequest) void
        +withdraw(Long) void
        +validateEmail(String) void
        +loadUserByEmail(String) Member
    }

    class OAuthMemberService {
        +processOAuthUser(String email, String providerId, AuthProvider) TokenResponse
    }

    class CustomOAuth2UserService {
        +loadUser(OAuth2UserRequest) OAuth2User
    }

    class OAuth2AuthenticationSuccessHandler {
        +onAuthenticationSuccess(request, response, authentication) void
    }

    class OAuth2AuthenticationFailureHandler {
        +onAuthenticationFailure(request, response, exception) void
    }

    class KakaoOAuthService {
        +getKakaoToken(String code) String
        +getKakaoUserInfo(String accessToken) KakaoUserInfo
    }

    class JwtAuthenticationFilter {
        +doFilterInternal(request, response, filterChain) void
    }

    class CustomUserDetails {
        +Long id
        +String email
        +getAuthorities() Collection
    }

    class CustomUserDetailsService {
        +loadUserByUsername(String email) UserDetails
    }

    class SecurityConfig {
        +filterChain(HttpSecurity) SecurityFilterChain
        +passwordEncoder() PasswordEncoder
    }

    class ProductService {
        +getProductList(Pageable, keyword) Page
        +getProductDetail(Long) ProductResponse
        +searchProducts(keyword, Pageable) Page
        +increaseViewCount(Long) void
        +createProduct(ProductRequest) void
        +updateProduct(Long, ProductRequest) void
        +deleteProduct(Long) void
    }

    class CartService {
        +getCart(Long) CartResponse
        +addItem(CartItemRequest, Long) void
        +updateQuantity(CartUpdateRequest, Long) void
        +removeItem(Long, Long) void
    }

    class OrderService {
        +createOrder(OrderRequest, Long) OrderResponse
        +cancelOrder(Long, Long) void
        +getOrderList(Long, Pageable) Page
        +getOrderDetail(Long, Long) OrderDetailResponse
        +validateOwnership(Long, Long) void
    }

    class PaymentService {
        +requestPayment(PaymentRequest, Long) PaymentReadyResponse
        +completePayment(Long, String) void
        +cancelPayment(Long, String) void
        +routeGateway(String) PaymentGateway
        +validateAmount(Long, Integer) void
    }

    class DeliveryService {
        +getDeliveryStatus(Long, Long) DeliveryResponse
        +updateDeliveryStatus(Long, DeliveryStatus) void
        +registerTracking(Long, TrackingRequest) void
    }

    class ReviewService {
        +writeReview(ReviewRequest, Long) void
        +updateReview(Long, ReviewRequest, Long) void
        +deleteReview(Long, Long) void
        +addReaction(Long, String, Long) void
        +validatePurchaser(Long, Long) void
    }

    class QnaService {
        +writeQuestion(QnaRequest, Long) void
        +answerQuestion(Long, AnswerRequest, Long) void
        +updateQuestion(Long, QnaRequest, Long) void
        +deleteQuestion(Long, Long) void
    }

    class NoticeService {
        +getNoticeList(Pageable) Page
        +getNoticeDetail(Long) NoticeResponse
        +createNotice(NoticeRequest, Long) void
        +updateNotice(Long, NoticeRequest) void
        +deleteNotice(Long) void
    }

    class JwtUtil {
        +generateAccessToken(Long, String) String
        +generateRefreshToken(Long) String
        +validateToken(String) boolean
        +getMemberId(String) Long
    }

    class MemberRepository {
        <<interface>>
        +findByEmail(String) Optional~Member~
        +existsByEmail(String) boolean
    }

    class RefreshTokenRepository {
        <<interface>>
        +findByToken(String) Optional~RefreshToken~
        +findByMemberId(Long) Optional~RefreshToken~
        +deleteByMemberId(Long) void
    }

    class ProductRepository {
        <<interface>>
        +findAllByStatusAndKeyword(Pageable, String) Page
        +findByIdAndStatus(Long, ProductStatus) Optional
    }

    class CartRepository {
        <<interface>>
        +findByMemberId(Long) Optional~Cart~
    }

    class OrderRepository {
        <<interface>>
        +findByMemberId(Long, Pageable) Page
        +findByIdAndMemberId(Long, Long) Optional
    }

    class PaymentRepository {
        <<interface>>
        +findByOrderId(Long) Optional~Payment~
    }

    class DeliveryRepository {
        <<interface>>
        +findByOrderId(Long) Optional~Delivery~
        +findAllByStatus(DeliveryStatus, Pageable) Page
    }

    class ReviewRepository {
        <<interface>>
        +findByMemberId(Long, Pageable) Page
        +existsByMemberIdAndProductId(Long, Long) boolean
    }

    class QnaRepository {
        <<interface>>
        +findByMemberIdAndDepth(Long, Integer, Pageable) Page
        +findByIdAndDepth(Long, Integer) Optional
    }

    class NoticeRepository {
        <<interface>>
        +findAllByOrderByIsFixedDescCreatedAtDesc(Pageable) Page
        +findByIdAndDeletedAtIsNull(Long) Optional
    }

    class CategoryRepository {
        <<interface>>
        +findAllTopByStatus(CategoryStatus) List
        +findAllSubByTopIdAndStatus(Long, CategoryStatus) List
    }

    class ProductImageRepository {
        <<interface>>
        +findAllByProductId(Long) List
        +deleteAllByProductId(Long) void
    }

    class AdminService {
        +getDashboard() DashboardResponse
        +getMemberList(Pageable, String) Page
        +getMemberDetail(Long) MemberResponse
        +changeMemberStatus(Long, MemberStatus) void
    }

    class CategoryService {
        +getTopCategoryList() List
        +getSubCategoryList(Long) List
        +createTopCategory(CategoryRequest) void
        +updateTopCategory(Long, CategoryRequest) void
        +deleteTopCategory(Long) void
        +createSubCategory(Long, CategoryRequest) void
        +updateSubCategory(Long, CategoryRequest) void
        +deleteSubCategory(Long) void
    }

    class PaymentGateway {
        <<interface>>
        +ready(PaymentReadyRequest) PaymentReadyResult
        +confirm(PaymentConfirmRequest) PaymentConfirmResult
        +cancel(PaymentCancelRequest) PaymentCancelResult
    }

    class TossPaymentGateway {
        -tossApiKey String
        -tossApiUrl String
        +ready(PaymentReadyRequest) PaymentReadyResult
        +confirm(PaymentConfirmRequest) PaymentConfirmResult
        +cancel(PaymentCancelRequest) PaymentCancelResult
    }

    class KakaoPaymentGateway {
        -kakaoAdminKey String
        -kakaoApiUrl String
        +ready(PaymentReadyRequest) PaymentReadyResult
        +confirm(PaymentConfirmRequest) PaymentConfirmResult
        +cancel(PaymentCancelRequest) PaymentCancelResult
    }

    class NaverPaymentGateway {
        -naverClientId String
        -naverClientSecret String
        -naverApiUrl String
        +ready(PaymentReadyRequest) PaymentReadyResult
        +confirm(PaymentConfirmRequest) PaymentConfirmResult
        +cancel(PaymentCancelRequest) PaymentCancelResult
    }

    %% 일반 회원 Controller → Service
    AuthController --> MemberService
    KakaoOAuthController --> KakaoOAuthService
    KakaoOAuthController --> OAuthMemberService
    MemberController --> MemberService
    ProductController --> ProductService
    CartController --> CartService
    OrderController --> OrderService
    PaymentController --> PaymentService
    DeliveryController --> DeliveryService
    ReviewController --> ReviewService
    QnaController --> QnaService
    NoticeController --> NoticeService

    %% 관리자 Controller → Service
    AdminController --> AdminService
    AdminProductController --> ProductService
    AdminCategoryController --> CategoryService
    AdminOrderController --> OrderService
    AdminPaymentController --> PaymentService
    AdminDeliveryController --> DeliveryService
    AdminReviewController --> ReviewService
    AdminQnaController --> QnaService
    AdminNoticeController --> NoticeService

    %% Service → Repository
    MemberService --> MemberRepository
    MemberService --> RefreshTokenRepository
    MemberService --> JwtUtil
    OAuthMemberService --> MemberRepository
    OAuthMemberService --> RefreshTokenRepository
    OAuthMemberService --> JwtUtil
    CustomOAuth2UserService --> OAuthMemberService
    OAuth2AuthenticationSuccessHandler --> JwtUtil
    KakaoOAuthService --> OAuthMemberService
    JwtAuthenticationFilter --> JwtUtil
    CustomUserDetailsService --> MemberRepository
    ProductService --> ProductRepository
    ProductService --> ProductImageRepository
    CategoryService --> CategoryRepository
    CartService --> CartRepository
    OrderService --> OrderRepository
    OrderService --> ProductRepository
    PaymentService --> PaymentRepository
    PaymentService --> OrderRepository
    PaymentService --> PaymentGateway
    AdminService --> MemberRepository
    AdminService --> OrderRepository
    AdminService --> QnaRepository
    AdminService --> ProductRepository
    DeliveryService --> DeliveryRepository
    DeliveryService --> OrderRepository
    ReviewService --> ReviewRepository
    ReviewService --> OrderRepository
    QnaService --> QnaRepository
    NoticeService --> NoticeRepository

    %% PaymentGateway 구현체
    PaymentGateway <|.. TossPaymentGateway : implements
    PaymentGateway <|.. KakaoPaymentGateway : implements
    PaymentGateway <|.. NaverPaymentGateway : implements
```

---

## 🔄 2. 시퀀스 다이어그램

---

### 2-1. 회원가입 (UC-M-001 / FM-001)

```mermaid
sequenceDiagram
    actor Client as Vue.js 클라이언트
    participant AC as AuthController
    participant MS as MemberService
    participant MR as MemberRepository
    participant DB as PostgreSQL

    Client->>AC: POST /signup { email, password, name, phone, address }
    AC->>MS: signup(SignupRequest)
    MS->>MR: existsByEmail(email)
    MR->>DB: SELECT * FROM member WHERE email = ?
    DB-->>MR: result
    alt 이메일 중복
        MR-->>MS: true
        MS-->>AC: throw DuplicateEmailException
        AC-->>Client: 409 Conflict "이미 사용 중인 이메일입니다."
    else 사용 가능
        MR-->>MS: false
        MS->>MS: BCrypt.encode(password)
        MS->>MR: save(Member)
        MR->>DB: INSERT INTO member (email, password, name, phone, address, member_role, member_status)
        DB-->>MR: saved Member
        MR-->>MS: Member
        MS-->>AC: void
        AC-->>Client: 201 Created "회원가입이 완료되었습니다."
    end
```

---

### 2-2. 로그인 & JWT 발급 (UC-M-002 / FM-002)

```mermaid
sequenceDiagram
    actor Client as Vue.js 클라이언트
    participant AC as AuthController
    participant MS as MemberService
    participant JU as JwtUtil
    participant MR as MemberRepository
    participant DB as PostgreSQL

    Client->>AC: POST /signin { email, password }
    AC->>MS: signin(SigninRequest)
    MS->>MR: findByEmail(email)
    MR->>DB: SELECT * FROM member WHERE email = ?
    DB-->>MR: Member | null
    alt 회원 미존재
        MR-->>MS: Optional.empty()
        MS-->>AC: throw MemberNotFoundException
        AC-->>Client: 401 Unauthorized "이메일 또는 비밀번호가 올바르지 않습니다."
    else 회원 존재
        MR-->>MS: Optional[Member]
        MS->>MS: BCrypt.matches(rawPw, encodedPw)
        alt 비밀번호 불일치
            MS-->>AC: throw InvalidPasswordException
            AC-->>Client: 401 Unauthorized
        else 비밀번호 일치
            MS->>MS: memberStatus == ACTIVATE 확인
            alt 비활성/탈퇴 계정
                MS-->>AC: throw InactiveMemberException
                AC-->>Client: 403 Forbidden "접근이 제한된 계정입니다."
            else 정상 계정
                MS->>JU: generateAccessToken(memberId, role)
                JU-->>MS: accessToken (단기)
                MS->>JU: generateRefreshToken(memberId)
                JU-->>MS: refreshToken (장기)
                MS-->>AC: TokenResponse { accessToken, refreshToken }
                AC-->>Client: 200 OK { accessToken, refreshToken }
            end
        end
    end
```

---

### 2-3. 주문 생성 (UC-M-010 / FO-001)

```mermaid
sequenceDiagram
    actor Client as Vue.js 클라이언트
    participant OC as OrderController
    participant OS as OrderService
    participant PR as ProductRepository
    participant CR as CartItemRepository
    participant OR as OrderRepository
    participant OIR as OrderItemRepository
    participant DR as DeliveryRepository
    participant DB as PostgreSQL

    Client->>OC: POST /order { cartItemIds[], deliveryAddress, receiver, phone }
    Note over OC: JWT 인증 → memberId 추출
    OC->>OS: createOrder(OrderRequest, memberId)

    OS->>CR: findCheckedItemsByIds(cartItemIds)
    CR->>DB: SELECT * FROM cart_item WHERE id IN (...)
    DB-->>CR: List~CartItem~
    CR-->>OS: List~CartItem~

    loop 각 CartItem에 대해
        OS->>PR: findByIdAndStatus(productId, ACTIVATE)
        PR->>DB: SELECT * FROM product WHERE id = ? AND product_status = 'ACTIVATE'
        DB-->>PR: Product
        PR-->>OS: Product
        OS->>OS: 재고(stock) >= quantity 확인
        alt 재고 부족
            OS-->>OC: throw OutOfStockException
            OC-->>Client: 409 Conflict "재고가 부족합니다."
        end
    end

    OS->>OS: 주문번호 생성 (ORD-YYYYMMDD-XXXXXX)
    OS->>OR: save(Order)
    OR->>DB: INSERT INTO order (member_id, number, order_status, ...)
    DB-->>OR: Order

    loop 각 상품별 스냅샷 저장
        OS->>OIR: save(OrderItem)
        OIR->>DB: INSERT INTO order_item (order_id, product_id, product_title, product_author, sale_price, quantity, item_total)
        DB-->>OIR: OrderItem
        OS->>PR: decreaseStock(productId, quantity)
        PR->>DB: UPDATE product SET stock = stock - ? WHERE id = ?
    end

    OS->>DR: save(Delivery)
    DR->>DB: INSERT INTO delivery (order_id, delivery_status='READY')

    OS->>CR: deleteAllByIds(cartItemIds)
    CR->>DB: DELETE FROM cart_item WHERE id IN (...)

    OR-->>OS: Order
    OS-->>OC: OrderResponse { orderId, orderNumber }
    OC-->>Client: 201 Created { orderId, orderNumber }
```

---

### 2-4. 결제 요청 & 완료 처리 (UC-M-011 / FPAY-001~002)

```mermaid
sequenceDiagram
    actor Client as Vue.js 클라이언트
    participant PC as PaymentController
    participant PS as PaymentService
    participant OR as OrderRepository
    participant PR as PaymentRepository
    participant DR as DeliveryRepository
    participant PG as PG 모듈(Mock)
    participant DB as PostgreSQL

    Client->>PC: POST /order/payment { orderId, paymentMethod, amount }
    Note over PC: JWT 인증 → memberId 추출
    PC->>PS: requestPayment(PaymentRequest, memberId)

    PS->>OR: findByIdAndMemberId(orderId, memberId)
    OR->>DB: SELECT * FROM order WHERE id = ? AND member_id = ?
    DB-->>OR: Order
    OR-->>PS: Order

    PS->>PS: order_status == PENDING 확인
    PS->>PS: amount == order.finalPrice 검증

    PS->>PR: save(Payment{status=READY})
    PR->>DB: INSERT INTO payment (order_id, method, payment_status='READY', amount)
    DB-->>PR: Payment
    PR-->>PS: Payment

    PS->>PG: requestPay(amount, orderId)
    PG-->>PS: pgTid (PG 트랜잭션 ID)

    PS-->>PC: PaymentReadyResponse { pgTid }
    PC-->>Client: 200 OK { pgTid }

    Note over Client: PG 창에서 결제 진행

    Client->>PC: POST /order/payment/complete { orderId, pgTid }
    PC->>PS: completePayment(orderId, pgTid)

    PS->>PG: verifyPayment(pgTid)
    alt PG 검증 실패
        PG-->>PS: FAILED
        PS->>PR: updateStatus(FAILED)
        PR->>DB: UPDATE payment SET payment_status = 'FAILED'
        PS-->>PC: throw PaymentFailedException
        PC-->>Client: 400 Bad Request "결제에 실패했습니다."
    else PG 검증 성공
        PG-->>PS: SUCCESS
        PS->>PR: updateStatus(PAID, pgTid, paidAt)
        PR->>DB: UPDATE payment SET payment_status = 'PAID', pg_tid = ?, paid_at = now()
        PS->>OR: updateStatus(PAYED)
        OR->>DB: UPDATE order SET order_status = 'PAYED'
        PS-->>PC: void
        PC-->>Client: 200 OK "결제가 완료되었습니다."
    end
```

---

### 2-5. 관리자 배송 상태 변경 (UC-A-008 / FA-014)

```mermaid
sequenceDiagram
    actor Admin as 관리자(Vue.js)
    participant ADC as AdminDeliveryController
    participant DS as DeliveryService
    participant DR as DeliveryRepository
    participant OR as OrderRepository
    participant DB as PostgreSQL

    Admin->>ADC: PATCH /admin/delivery/{id}/status { deliveryStatus }
    Note over ADC: JWT 인증 → MANAGER/ADMIN 권한 확인

    ADC->>DS: updateDeliveryStatus(deliveryId, deliveryStatus)
    DS->>DR: findById(deliveryId)
    DR->>DB: SELECT * FROM delivery WHERE id = ?
    DB-->>DR: Delivery
    DR-->>DS: Delivery

    DS->>DS: 상태 전이 유효성 확인
    Note over DS: READY→SHIPPED→IN_TRANSIT→DELIVERED 순서 강제

    alt 운송장 미등록 상태에서 SHIPPED 시도
        DS-->>ADC: throw TrackingNumberRequiredException
        ADC-->>Admin: 400 Bad Request "운송장 번호를 먼저 등록하세요."
    else 유효한 상태 전이
        DS->>DR: save(delivery.setStatus(deliveryStatus))
        DR->>DB: UPDATE delivery SET delivery_status = ?, updated_at = now()
        alt deliveryStatus == DELIVERED
            DS->>OR: updateStatus(APPROVAL)
            OR->>DB: UPDATE order SET order_status = 'APPROVAL'
            DS->>DR: save(delivery.setDeliveredAt(now()))
            DR->>DB: UPDATE delivery SET delivered_at = now()
        end
        DR-->>DS: Delivery
        DS-->>ADC: void
        ADC-->>Admin: 200 OK "배송 상태가 변경되었습니다."
    end
```

---

### 2-6. 리뷰 작성 (UC-M-015 / FR-002)

```mermaid
sequenceDiagram
    actor Client as Vue.js 클라이언트
    participant RC as ReviewController
    participant RS as ReviewService
    participant OIR as OrderItemRepository
    participant RR as ReviewRepository
    participant RIR as ReviewImageRepository
    participant DB as PostgreSQL

    Client->>RC: POST /review/write { productId, rating, content, images[] }
    Note over RC: JWT 인증 → memberId 추출
    RC->>RS: writeReview(ReviewRequest, memberId)

    RS->>OIR: findDeliveredItemByMemberAndProduct(memberId, productId)
    OIR->>DB: SELECT oi.* FROM order_item oi JOIN order o ON o.id = oi.order_id WHERE o.member_id = ? AND oi.product_id = ? AND o.order_status = 'APPROVAL' AND oi.is_reviewed = false
    DB-->>OIR: OrderItem | null

    alt 구매 이력 없음 / 이미 리뷰 작성
        OIR-->>RS: null
        RS-->>RC: throw ReviewNotAllowedException
        RC-->>Client: 403 Forbidden "리뷰 작성 권한이 없습니다."
    else 구매 이력 확인됨
        OIR-->>RS: OrderItem
        RS->>RR: existsByMemberIdAndProductId(memberId, productId)
        RR->>DB: SELECT COUNT(*) FROM review WHERE member_id = ? AND product_id = ? AND deleted_at IS NULL
        DB-->>RR: count

        alt 중복 리뷰
            RR-->>RS: true
            RS-->>RC: throw DuplicateReviewException
            RC-->>Client: 409 Conflict "이미 리뷰를 작성하셨습니다."
        else 최초 작성
            RR-->>RS: false
            RS->>RS: rating 1~5 범위 검증
            RS->>RR: save(Review)
            RR->>DB: INSERT INTO review (member_id, product_id, rating, content)
            DB-->>RR: Review

            loop 이미지 첨부 (최대 5장)
                RS->>RIR: save(ReviewImage)
                RIR->>DB: INSERT INTO review_image (review_id, image_url)
            end

            RS->>OIR: updateIsReviewed(orderItemId, true)
            OIR->>DB: UPDATE order_item SET is_reviewed = true WHERE id = ?

            RS-->>RC: void
            RC-->>Client: 201 Created "리뷰가 등록되었습니다."
        end
    end
```

---

### 2-7. QnA 질문 등록 & 관리자 답변 (UC-M-020 / UC-A-010)

```mermaid
sequenceDiagram
    actor Member as 회원(Vue.js)
    actor Admin as 관리자(Vue.js)
    participant QC as QnaController
    participant ADC as AdminQnaController
    participant QS as QnaService
    participant QR as QnaRepository
    participant DB as PostgreSQL

    Note over Member, DB: [1단계] 회원 질문 등록
    Member->>QC: POST /qna/write { title, content, category, isSecret, productId }
    Note over QC: JWT 인증 → memberId 추출
    QC->>QS: writeQuestion(QnaRequest, memberId)
    QS->>QR: save(Qna{depth=0, qnaStatus=WAITING})
    QR->>DB: INSERT INTO qna (member_id, depth=0, title, content, qna_status='WAITING', is_secret, parent_id=NULL)
    DB-->>QR: Qna
    QR-->>QS: Qna
    QS-->>QC: void
    QC-->>Member: 201 Created "문의가 등록되었습니다."

    Note over Admin, DB: [2단계] 관리자 답변 등록
    Admin->>ADC: POST /admin/qna/{id}/insert { content }
    Note over ADC: JWT 인증 → MANAGER/ADMIN 권한 확인
    ADC->>QS: answerQuestion(parentId, AnswerRequest, adminMemberId)

    QS->>QR: findByIdAndDepth(parentId, 0)
    QR->>DB: SELECT * FROM qna WHERE id = ? AND depth = 0 AND deleted_at IS NULL
    DB-->>QR: Qna(질문)
    QR-->>QS: Qna

    QS->>QS: qnaStatus == WAITING 또는 PROCESSING 확인
    alt 이미 COMPLETE 상태
        QS-->>ADC: throw AlreadyAnsweredException
        ADC-->>Admin: 409 Conflict "이미 답변이 완료된 문의입니다."
    else 답변 가능
        QS->>QR: save(Qna{depth=1, parentId=parentId, qnaStatus=COMPLETE})
        QR->>DB: INSERT INTO qna (member_id, parent_id=?, depth=1, title='RE:...', content, qna_status='COMPLETE', answered_at=now())
        DB-->>QR: Qna(답변)
        QS->>QR: updateStatus(parentId, COMPLETE)
        QR->>DB: UPDATE qna SET qna_status = 'COMPLETE', answered_at = now() WHERE id = ?
        DB-->>QR: updated
        QS-->>ADC: void
        ADC-->>Admin: 201 Created "답변이 등록되었습니다."
    end

    Note over Member: [3단계] 회원 답변 확인
    Member->>QC: GET /mypage/qna/{id}
    QC->>QS: getQnaDetail(qnaId, memberId)
    QS->>QR: findByIdWithChildren(qnaId)
    QR->>DB: SELECT * FROM qna WHERE id = ? OR parent_id = ? ORDER BY depth ASC
    DB-->>QR: List~Qna~ (질문 + 답변)
    QR-->>QS: List~Qna~
    QS->>QS: isSecret 권한 확인 (본인 또는 관리자만 열람)
    QS-->>QC: QnaDetailResponse
    QC-->>Member: 200 OK { question, answer, qnaStatus: COMPLETE }
```

---

### 2-8. 관리자 대시보드 조회 (UC-A-001 / FA-001)

```mermaid
sequenceDiagram
    actor Admin as 관리자(Vue.js)
    participant ADC as AdminController
    participant AS as AdminService
    participant OR as OrderRepository
    participant QR as QnaRepository
    participant PR as ProductRepository
    participant MR as MemberRepository
    participant DB as PostgreSQL

    Admin->>ADC: GET /admin
    Note over ADC: JWT 인증 → MANAGER/ADMIN 권한 확인
    ADC->>AS: getDashboard()

    par 미승인 주문 집계
        AS->>OR: findTopByStatus(PENDING, limit=10)
        OR->>DB: SELECT * FROM order WHERE order_status = 'PENDING' ORDER BY ordered_at DESC LIMIT 10
        DB-->>OR: List~Order~
    and 미답변 QnA 집계
        AS->>QR: findTopByStatus(WAITING, limit=10)
        QR->>DB: SELECT * FROM qna WHERE qna_status = 'WAITING' AND depth = 0 ORDER BY created_at DESC LIMIT 10
        DB-->>QR: List~Qna~
    and 재고 부족 상품 집계
        AS->>PR: findLowStock(threshold=10)
        PR->>DB: SELECT * FROM product WHERE stock <= 10 AND product_status = 'ACTIVATE' ORDER BY stock ASC
        DB-->>PR: List~Product~
    and 오늘/이번 달 매출 집계
        AS->>OR: sumFinalPriceByPeriod(TODAY, MONTH)
        OR->>DB: SELECT SUM(final_price) FROM order WHERE order_status IN ('PAYED','APPROVAL') AND ordered_at >= ?
        DB-->>OR: Long amount
    and 신규 회원 집계
        AS->>MR: countByCreatedAtPeriod(TODAY, MONTH)
        MR->>DB: SELECT COUNT(*) FROM member WHERE created_at >= ?
        DB-->>MR: Long count
    end

    AS-->>ADC: DashboardResponse { pendingOrders, waitingQnas, lowStockProducts, todaySales, monthSales, newMembersToday, newMembersMonth }
    ADC-->>Admin: 200 OK DashboardResponse
```

---

### 2-9. 로그아웃 (UC-M-003 / FM-003)

```mermaid
sequenceDiagram
    actor Client as Vue.js 클라이언트
    participant AC as AuthController
    participant MS as MemberService
    participant JU as JwtUtil

    Client->>AC: POST /signout
    Note over AC: Authorization 헤더에서 AccessToken 추출
    AC->>MS: signout(accessToken)
    MS->>JU: validateToken(accessToken)
    JU-->>MS: boolean

    alt 토큰 유효하지 않음
        MS-->>AC: throw InvalidTokenException
        AC-->>Client: 401 Unauthorized "유효하지 않은 토큰입니다."
    else 토큰 유효
        MS->>JU: getTokenRemainingTime(accessToken)
        JU-->>MS: remainingMillis
        MS->>MS: BlacklistToken 등록 (Redis / In-Memory)
        Note over MS: TTL = remainingMillis (자동 만료)
        MS-->>AC: void
        AC-->>Client: 200 OK "로그아웃 되었습니다."
    end

    Note over Client: 클라이언트에서 localStorage AccessToken / RefreshToken 삭제
```

---

### 2-10. 토스페이먼츠 결제 처리 (UC-M-011-T / FPAY-005~008)

```mermaid
sequenceDiagram
    actor Client as Vue.js 클라이언트
    participant PC as PaymentController
    participant PS as PaymentService
    participant TPG as TossPaymentGateway
    participant OR as OrderRepository
    participant PR as PaymentRepository
    participant TossAPI as 토스페이먼츠 API
    participant DB as PostgreSQL

    Note over Client, DB: [1단계] 결제 준비 (금액 검증 + READY 저장)
    Client->>PC: POST /payment/ready { orderId, amount, provider="TOSS", method }
    Note over PC: JWT 인증 → memberId 추출
    PC->>PS: requestPayment(PaymentRequest, memberId)

    PS->>OR: findByIdAndMemberId(orderId, memberId)
    OR->>DB: SELECT * FROM order WHERE id = ? AND member_id = ?
    DB-->>OR: Order
    OR-->>PS: Order

    PS->>PS: validateAmount(orderId, amount)
    Note over PS: order.finalPrice == request.amount 금액 위변조 방지

    alt 금액 불일치
        PS-->>PC: throw AmountTamperedException
        PC-->>Client: 400 Bad Request "결제 금액이 일치하지 않습니다."
    else 금액 일치
        PS->>PR: save(Payment{status=READY, provider=TOSS})
        PR->>DB: INSERT INTO payment (order_id, method, provider='TOSS', payment_status='READY', amount)
        DB-->>PR: Payment
        PS-->>PC: PaymentReadyResponse { paymentId, amount, orderName }
        PC-->>Client: 200 OK { paymentId, amount, orderName }
    end

    Note over Client: 토스페이먼츠 SDK로 결제창 호출

    Note over Client, DB: [2단계] 결제 승인 (서버 confirm)
    Client->>PC: POST /payment/toss/confirm { orderId, paymentKey, amount }
    PC->>PS: completePayment(orderId, paymentKey)

    PS->>PS: validateAmount(orderId, amount)
    Note over PS: 재검증 — 클라이언트 위변조 방지

    PS->>PS: routeGateway("TOSS")
    PS->>TPG: confirm(PaymentConfirmRequest{paymentKey, orderId, amount})
    TPG->>TossAPI: POST /confirm { paymentKey, orderId, amount }
    TossAPI-->>TPG: 승인 결과 (success / fail)

    alt 승인 실패
        TPG-->>PS: PaymentConfirmResult{success=false, failureReason}
        PS->>PR: updateStatus(FAILED, failureReason)
        PR->>DB: UPDATE payment SET payment_status='FAILED', failure_reason=?
        PS-->>PC: throw PaymentConfirmFailedException
        PC-->>Client: 400 Bad Request "결제 승인에 실패했습니다."
    else 승인 성공
        TPG-->>PS: PaymentConfirmResult{success=true, paymentKey, paidAt}
        PS->>PR: updateStatus(PAID, paymentKey, paidAt)
        PR->>DB: UPDATE payment SET payment_status='PAID', payment_key=?, paid_at=?
        PS->>OR: updateStatus(PAYED)
        OR->>DB: UPDATE order SET order_status='PAYED'
        PS-->>PC: void
        PC-->>Client: 200 OK "결제가 완료되었습니다."
    end
```

---

### 2-11. 카카오페이 결제 처리 (UC-M-011-K / FPAY-009~011)

```mermaid
sequenceDiagram
    actor Client as Vue.js 클라이언트
    participant PC as PaymentController
    participant PS as PaymentService
    participant KPG as KakaoPaymentGateway
    participant OR as OrderRepository
    participant PR as PaymentRepository
    participant KakaoAPI as 카카오페이 API
    participant DB as PostgreSQL

    Note over Client, DB: [1단계] 결제 ready
    Client->>PC: POST /payment/kakao/ready { orderId, amount, method }
    Note over PC: JWT 인증 → memberId 추출
    PC->>PS: requestPayment(PaymentRequest, memberId)

    PS->>PS: validateAmount(orderId, amount)

    PS->>PS: routeGateway("KAKAO")
    PS->>KPG: ready(PaymentReadyRequest{orderId, amount, itemName})
    KPG->>KakaoAPI: POST /ready { cid, partner_order_id, partner_user_id, item_name, quantity, total_amount, ... }
    KakaoAPI-->>KPG: { tid, next_redirect_pc_url }
    KPG-->>PS: PaymentReadyResult{tid, redirectUrl}

    PS->>PR: save(Payment{status=READY, provider=KAKAO, pgTid=tid})
    PR->>DB: INSERT INTO payment (..., provider='KAKAO', pg_tid=?, payment_status='READY')
    PS-->>PC: PaymentReadyResponse{tid, redirectUrl}
    PC-->>Client: 200 OK { redirectUrl }

    Note over Client: 카카오페이 결제 페이지로 리다이렉트 후 결제 진행

    Note over Client, DB: [2단계] 결제 approve (사용자 승인 후 콜백)
    Client->>PC: GET /payment/kakao/approve?pg_token=xxx&orderId=yyy
    PC->>PS: completePayment(orderId, pgToken)

    PS->>PR: findByOrderId(orderId)
    PR->>DB: SELECT * FROM payment WHERE order_id = ?
    DB-->>PR: Payment{pgTid}

    PS->>KPG: confirm(PaymentConfirmRequest{tid, pgToken, orderId})
    KPG->>KakaoAPI: POST /approve { cid, tid, partner_order_id, pg_token }
    KakaoAPI-->>KPG: 승인 결과

    alt 승인 실패
        KPG-->>PS: PaymentConfirmResult{success=false}
        PS->>PR: updateStatus(FAILED)
        PR->>DB: UPDATE payment SET payment_status='FAILED'
        PS-->>PC: throw PaymentConfirmFailedException
        PC-->>Client: 400 Bad Request "카카오페이 결제 승인에 실패했습니다."
    else 승인 성공
        KPG-->>PS: PaymentConfirmResult{success=true, tid, approvedAt}
        PS->>PR: updateStatus(PAID, tid, approvedAt)
        PR->>DB: UPDATE payment SET payment_status='PAID', paid_at=?
        PS->>OR: updateStatus(PAYED)
        OR->>DB: UPDATE order SET order_status='PAYED'
        PS-->>PC: void
        PC-->>Client: 302 Redirect → 결제 완료 페이지
    end
```

---

### 2-12. 네이버페이 결제 처리 (UC-M-011-N / FPAY-012~014)

```mermaid
sequenceDiagram
    actor Client as Vue.js 클라이언트
    participant PC as PaymentController
    participant PS as PaymentService
    participant NPG as NaverPaymentGateway
    participant OR as OrderRepository
    participant PR as PaymentRepository
    participant NaverAPI as 네이버페이 API
    participant DB as PostgreSQL

    Note over Client, DB: [1단계] 결제 준비
    Client->>PC: POST /payment/naver/ready { orderId, amount, method }
    Note over PC: JWT 인증 → memberId 추출
    PC->>PS: requestPayment(PaymentRequest, memberId)

    PS->>PS: validateAmount(orderId, amount)

    PS->>PS: routeGateway("NAVER")
    PS->>NPG: ready(PaymentReadyRequest{orderId, amount, productName})
    NPG->>NaverAPI: POST /payments/apply { merchantPayKey, productName, totalPayAmount, ... }
    NaverAPI-->>NPG: { paymentId, checkoutPage }
    NPG-->>PS: PaymentReadyResult{paymentId, checkoutPage}

    PS->>PR: save(Payment{status=READY, provider=NAVER, pgTid=paymentId})
    PR->>DB: INSERT INTO payment (..., provider='NAVER', pg_tid=?, payment_status='READY')
    PS-->>PC: PaymentReadyResponse{paymentId, checkoutPage}
    PC-->>Client: 200 OK { checkoutPage }

    Note over Client: 네이버페이 결제 창으로 리다이렉트

    Note over Client, DB: [2단계] 결제 승인
    Client->>PC: POST /payment/naver/approve { orderId, paymentId }
    PC->>PS: completePayment(orderId, paymentId)

    PS->>NPG: confirm(PaymentConfirmRequest{paymentId, orderId})
    NPG->>NaverAPI: POST /payments/apply/result { paymentId }
    NaverAPI-->>NPG: 승인 결과

    alt 승인 실패
        NPG-->>PS: PaymentConfirmResult{success=false}
        PS->>PR: updateStatus(FAILED)
        PR->>DB: UPDATE payment SET payment_status='FAILED'
        PS-->>PC: throw PaymentConfirmFailedException
        PC-->>Client: 400 Bad Request "네이버페이 결제 승인에 실패했습니다."
    else 승인 성공
        NPG-->>PS: PaymentConfirmResult{success=true, paymentId, approvedAt}
        PS->>PR: updateStatus(PAID, paymentId, approvedAt)
        PR->>DB: UPDATE payment SET payment_status='PAID', paid_at=?
        PS->>OR: updateStatus(PAYED)
        OR->>DB: UPDATE order SET order_status='PAYED'
        PS-->>PC: void
        PC-->>Client: 200 OK "결제가 완료되었습니다."
    end
```

---

### 2-13. PG사별 결제 취소/환불 처리 (UC-M-012-PG / FPAY-008/011/014)

```mermaid
sequenceDiagram
    actor Client as Vue.js 클라이언트
    participant MC as MemberController
    participant PS as PaymentService
    participant PR as PaymentRepository
    participant OR as OrderRepository
    participant GW as PaymentGateway (구현체 분기)
    participant PGAPI as PG사 API (Toss / Kakao / Naver)
    participant DB as PostgreSQL

    Client->>MC: POST /mypage/payment/{paymentId}/cancel { cancelReason }
    Note over MC: JWT 인증 → memberId 추출
    MC->>PS: cancelPayment(paymentId, cancelReason)

    PS->>PR: findById(paymentId)
    PR->>DB: SELECT * FROM payment WHERE id = ?
    DB-->>PR: Payment{provider, paymentKey/pgTid, paymentStatus, amount}
    PR-->>PS: Payment

    PS->>PS: paymentStatus == PAID 확인
    alt 취소 불가 상태 (CANCELLED / FAILED / READY)
        PS-->>MC: throw InvalidPaymentStatusException
        MC-->>Client: 409 Conflict "취소할 수 없는 결제 상태입니다."
    else PAID 상태
        PS->>PS: routeGateway(payment.provider)
        Note over PS: TOSS→TossPaymentGateway / KAKAO→KakaoPaymentGateway / NAVER→NaverPaymentGateway

        PS->>GW: cancel(PaymentCancelRequest{pgTid, cancelReason, cancelAmount})
        GW->>PGAPI: 취소 API 호출 (provider별 엔드포인트)
        PGAPI-->>GW: 취소 결과

        alt PG사 취소 실패
            GW-->>PS: PaymentCancelResult{success=false, reason}
            PS-->>MC: throw PaymentCancelFailedException
            MC-->>Client: 500 Internal Server Error "PG사 취소 처리에 실패했습니다."
        else PG사 취소 성공
            GW-->>PS: PaymentCancelResult{success=true, cancelledAt}
            PS->>PR: updateStatus(CANCELLED, cancelReason, cancelledAt)
            PR->>DB: UPDATE payment SET payment_status='CANCELLED', cancel_reason=?, cancelled_at=?
            PS->>OR: updateStatus(CANCELED)
            OR->>DB: UPDATE order SET order_status='CANCELED'
            PS-->>MC: void
            MC-->>Client: 200 OK "결제가 취소되었습니다."
        end
    end
```
```

---

### 2-14. 토큰 갱신 — RefreshToken → AccessToken 재발급 (v1.3 신규)

```mermaid
sequenceDiagram
    actor Client as Vue.js 클라이언트
    participant AC as AuthController
    participant MS as MemberService
    participant RTR as RefreshTokenRepository
    participant JU as JwtUtil
    participant DB as PostgreSQL

    Client->>AC: POST /auth/refresh { refreshToken }
    AC->>MS: refresh(refreshToken)

    MS->>JU: validateToken(refreshToken)
    alt 토큰 만료 또는 형식 오류
        JU-->>MS: false
        MS-->>AC: throw InvalidTokenException
        AC-->>Client: 401 Unauthorized "토큰이 만료되었습니다."
    else 유효한 토큰
        JU-->>MS: true
        MS->>JU: getMemberId(refreshToken)
        JU-->>MS: memberId
        MS->>RTR: findByToken(refreshToken)
        RTR->>DB: SELECT * FROM refresh_token WHERE token = ?
        DB-->>RTR: RefreshToken | null
        alt DB에 없는 토큰 (탈취 감지)
            RTR-->>MS: empty
            MS->>RTR: deleteByMemberId(memberId)
            RTR->>DB: DELETE FROM refresh_token WHERE member_id = ?
            MS-->>AC: throw TokenStealingException
            AC-->>Client: 401 Unauthorized "비정상 접근 감지. 재로그인 필요"
        else 정상 토큰
            RTR-->>MS: RefreshToken
            Note over MS: Rotation — 기존 토큰 삭제 후 신규 발급
            MS->>JU: generateAccessToken(memberId, role)
            JU-->>MS: newAccessToken
            MS->>JU: generateRefreshToken(memberId)
            JU-->>MS: newRefreshToken
            MS->>RTR: deleteByMemberId(memberId)
            RTR->>DB: DELETE FROM refresh_token WHERE member_id = ?
            MS->>RTR: save(RefreshToken{memberId, newRefreshToken, expiresAt})
            RTR->>DB: INSERT INTO refresh_token (member_id, token, expires_at)
            MS-->>AC: TokenResponse { newAccessToken, newRefreshToken }
            AC-->>Client: 200 OK { accessToken, refreshToken }
        end
    end
```

---

### 2-15. 구글 OAuth 로그인 (v1.3 신규)

```mermaid
sequenceDiagram
    actor Client as Vue.js 클라이언트
    participant AC as AuthController
    participant SP as Spring Security OAuth2
    participant GG as Google OAuth Server
    participant COS as CustomOAuth2UserService
    participant OMS as OAuthMemberService
    participant MR as MemberRepository
    participant RTR as RefreshTokenRepository
    participant JU as JwtUtil
    participant DB as PostgreSQL

    Client->>SP: GET /oauth2/authorization/google
    SP-->>Client: 302 Redirect → Google 로그인 페이지

    Client->>GG: 사용자 구글 계정 로그인 + 권한 동의
    GG-->>SP: Authorization Code 전달 (/login/oauth2/code/google)

    SP->>GG: Authorization Code → Access Token 교환
    GG-->>SP: Google Access Token

    SP->>COS: loadUser(OAuth2UserRequest)
    COS->>GG: Google API 사용자 정보 요청
    GG-->>COS: { email, name, sub(providerId) }

    COS->>OMS: processOAuthUser(email, providerId, GOOGLE)
    OMS->>MR: findByEmail(email)
    MR->>DB: SELECT * FROM member WHERE email = ?
    DB-->>MR: Member | null

    alt 신규 회원
        MR-->>OMS: empty
        OMS->>MR: save(Member{email, name, provider=GOOGLE, providerId})
        MR->>DB: INSERT INTO member (email, name, provider, provider_id)
        DB-->>MR: saved Member
    else 기존 회원 (LOCAL 가입자가 구글 재시도)
        MR-->>OMS: Member{provider=LOCAL}
        OMS-->>COS: throw OAuthProviderMismatchException
    else 기존 구글 회원
        MR-->>OMS: Member{provider=GOOGLE}
    end

    OMS->>JU: generateAccessToken(memberId, role)
    JU-->>OMS: accessToken
    OMS->>JU: generateRefreshToken(memberId)
    JU-->>OMS: refreshToken
    OMS->>RTR: save(RefreshToken{memberId, refreshToken, expiresAt})
    RTR->>DB: INSERT INTO refresh_token (member_id, token, expires_at)
    OMS-->>COS: TokenResponse

    Note over SP: OAuth2AuthenticationSuccessHandler 동작
    SP-->>Client: 302 Redirect → /oauth/callback?accessToken=...&refreshToken=...
    Note over Client: Vue OAuthCallbackView에서 토큰 저장 후 메인 이동
```

---

### 2-16. 카카오 OAuth 로그인 (v1.3 신규)

```mermaid
sequenceDiagram
    actor Client as Vue.js 클라이언트
    participant KOC as KakaoOAuthController
    participant KOS as KakaoOAuthService
    participant KK as Kakao API Server
    participant OMS as OAuthMemberService
    participant MR as MemberRepository
    participant RTR as RefreshTokenRepository
    participant JU as JwtUtil
    participant DB as PostgreSQL

    Client->>KOC: GET /oauth/kakao/authorize
    KOC-->>Client: 302 Redirect → Kakao 로그인 페이지

    Client->>KK: 사용자 카카오 계정 로그인 + 권한 동의
    KK-->>Client: 302 Redirect → /oauth/kakao/callback?code=...

    Client->>KOC: GET /oauth/kakao/callback?code={authCode}
    KOC->>KOS: getKakaoToken(authCode)

    KOS->>KK: POST https://kauth.kakao.com/oauth/token { code, client_id, redirect_uri }
    KK-->>KOS: { access_token, refresh_token }

    KOS->>KK: GET https://kapi.kakao.com/v2/user/me (Authorization: Bearer access_token)
    KK-->>KOS: { id, kakao_account.email }
    KOS-->>KOC: KakaoUserInfo { id, email }

    KOC->>OMS: processOAuthUser(email, id(String), KAKAO)
    OMS->>MR: findByEmail(email)
    MR->>DB: SELECT * FROM member WHERE email = ?
    DB-->>MR: Member | null

    alt 신규 회원
        MR-->>OMS: empty
        OMS->>MR: save(Member{email, provider=KAKAO, providerId=id})
        MR->>DB: INSERT INTO member (email, provider, provider_id)
        DB-->>MR: saved Member
    else 기존 회원 (provider 불일치)
        MR-->>OMS: Member{provider=LOCAL 또는 GOOGLE}
        OMS-->>KOC: throw OAuthProviderMismatchException
        KOC-->>Client: 302 Redirect → /login?error=provider_mismatch
    else 기존 카카오 회원
        MR-->>OMS: Member{provider=KAKAO}
    end

    OMS->>JU: generateAccessToken(memberId, role)
    JU-->>OMS: accessToken
    OMS->>JU: generateRefreshToken(memberId)
    JU-->>OMS: refreshToken
    OMS->>RTR: save(RefreshToken{memberId, refreshToken, expiresAt})
    RTR->>DB: INSERT INTO refresh_token (member_id, token, expires_at)
    OMS-->>KOC: TokenResponse { accessToken, refreshToken }

    KOC-->>Client: 302 Redirect → /oauth/callback?accessToken=...&refreshToken=...
    Note over Client: Vue OAuthCallbackView에서 토큰 저장 후 메인 이동
```
