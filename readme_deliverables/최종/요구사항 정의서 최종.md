...
| v1.3 | 2026.03.29 | - | JWT 인증 / 구글 OAuth / 카카오 OAuth 요구사항 신규 추가 (REQ-AUTH-001~005) / 비기능 요구사항 인증 항목 3건 추가 |
...
### 4.1.7 인증 — JWT / OAuth (v1.3 신규)

| 요구사항 ID | 기능명 | 설명 | 우선순위 | 관련 페이지 | 비고 |
| --- | --- | --- | --- | --- | --- |
| REQ-AUTH-001 | JWT 기반 Stateless 인증 | AccessToken(30분) + RefreshToken(7일) 발급, 세션 미사용 | 🔴 높음 | - | Spring Security + JwtAuthenticationFilter 구현 |
| REQ-AUTH-002 | RefreshToken 자동 갱신 | AccessToken 만료 시 RefreshToken으로 재발급, Rotation 전략 적용 | 🔴 높음 | /auth/refresh | 기존 토큰 즉시 삭제 후 신규 발급 (탈취 감지) |
| REQ-AUTH-003 | 구글 OAuth 로그인 | 구글 계정으로 소셜 로그인, 신규 회원 자동 가입 처리 | 🔴 높음 | /oauth2/authorization/google | spring-boot-starter-oauth2-client 사용 |
| REQ-AUTH-004 | 카카오 OAuth 로그인 | 카카오 계정으로 소셜 로그인, 수동 REST API 구현 | 🔴 높음 | /oauth/kakao/authorize | 이메일 없는 카카오 사용자 처리 포함 |
| REQ-AUTH-005 | OAuth 제공자 불일치 방지 | 동일 이메일로 다른 provider 로그인 시도 시 오류 반환 | 🔴 높음 | - | OAuthProviderMismatchException 처리 |
...
| REQ-NFR-AUTH-001 | 토큰 보안 | AccessToken 메모리 저장, RefreshToken HttpOnly 쿠키 또는 localStorage 저장, 전송 시 Bearer 헤더 사용 |
| REQ-NFR-AUTH-002 | RefreshToken 탈취 감지 | DB에 없는 RefreshToken 사용 시 해당 회원의 모든 토큰 즉시 삭제 (Rotation 전략) |
| REQ-NFR-AUTH-003 | OAuth 시크릿 서버 보관 | Google clientSecret, Kakao adminKey를 서버 환경변수로만 관리, 클라이언트 노출 금지 |
