...
classDiagram
    ...
    class com.shop.domain {
        +Member / MemberRole / MemberStatus / AuthProvider
        +RefreshToken
    ...
    com.shop.config --> com.shop.oauth2
    com.shop.controller --> com.shop.util
    com.shop.service --> com.shop.util
    com.shop.oauth2 --> com.shop.service
    com.shop.oauth2 --> com.shop.util
```
...
│   ├── main/
│   │   ├── java/com/shop/
│   │   │   ├── config/          ← 보안·설정 (SecurityConfig, WebMvcConfig)
│   │   │   ├── controller/      ← REST API 엔드포인트 (AuthController, KakaoOAuthController 포함)
│   │   │   ├── service/         ← 비즈니스 로직 (MemberService, OAuthMemberService, KakaoOAuthService 포함)
│   │   │   ├── repository/      ← JPA DB 접근 (RefreshTokenRepository 포함)
│   │   │   ├── domain/          ← Entity & Enum (Member, RefreshToken, AuthProvider 포함)
│   │   │   ├── dto/             ← 요청/응답 객체
│   │   │   ├── security/        ← JWT 인증 처리 (JwtAuthenticationFilter, CustomUserDetails, CustomUserDetailsService)
│   │   │   ├── oauth2/          ← OAuth2 처리 (CustomOAuth2UserService, SuccessHandler, FailureHandler) ← v1.3 신규
│   │   │   ├── util/            ← 공통 유틸리티 (JwtUtil 포함)
...
│   └── index.html
├── src/
│   ├── api/                 ← Axios API 호출 모듈 (axios.js 인터셉터 + 401 자동 갱신)
│   │   ├── axios.js         ← Axios 인스턴스 + 토큰 인터셉터
│   │   ├── auth.js          ← 로그인/로그아웃/토큰 갱신 API
│   │   ├── product.js
│   │   └── order.js
│   ├── assets/              ← 이미지·전역 CSS
│   ├── components/          ← 재사용 컴포넌트
...
│   │   ├── admin/
│   │   ├── member/
│   │   │   ├── SigninView.vue         ← 로그인 (일반 + 소셜 버튼)
│   │   │   └── OAuthCallbackView.vue ← OAuth 콜백 처리 (토큰 저장) ← v1.3 신규
│   │   ├── product/
│   │   ├── cart/
│   │   ├── order/
│   │   ├── payment/
...
| `security` | JWT 토큰 발급·검증, Spring Security 인증 필터(`JwtAuthenticationFilter`), UserDetails 구현체(`CustomUserDetails`, `CustomUserDetailsService`)가 위치합니다. |
| `oauth2` | 구글 OAuth2 처리 클래스(`CustomOAuth2UserService`, `OAuth2AuthenticationSuccessHandler`, `OAuth2AuthenticationFailureHandler`)가 위치합니다. v1.3 신규 추가. |
